...
# EnhancedSRUnet model (replacing old SRUnet)
...
class SelfEnsembleWrapper(nn.Module):
    ...
    def forward(self, x):
        out = self.model(x)
        for flip_dim in [2, 3, None]:
            if flip_dim is not None:
                x_aug = torch.flip(x, [flip_dim])
            else:
                x_aug = x.rot90(1, [2, 3])
            out_aug = self.model(x_aug)
            if flip_dim is not None:
                out_aug = torch.flip(out_aug, [flip_dim])
            else:
                out_aug = out_aug.rot90(-1, [2, 3])
            out = out + out_aug
        return out / 5
...
class HybridModel(nn.Module):
    def __init__(self):
        ...
        self.lefuse = LEFuse()
        self.unet = EnhancedSRUnet()
        self.ensemble = SelfEnsembleWrapper(self.unet)

    def forward(self, rgb, ir):
        y, cr, cb = RGB2YCrCb(rgb)
        fused_y = self.lefuse(y, ir)
        fused_ycbcr = torch.cat([fused_y, cb, cr], dim=1)
        fused_rgb_lr = YCbCr2RGB(fused_y, cr, cb)
        fused_rgb_hr = self.ensemble(fused_rgb_lr)
        return fused_rgb_hr

# Device verification and model test
if __name__ == "__main__":
    ...
    # Test forward pass
    dummy_rgb = torch.rand(1, 3, 64, 64, device=device)
    dummy_ir = torch.rand(1, 1, 64, 64, device=device)
    output = model(dummy_rgb, dummy_ir)
    print(f"RGB input shape: {dummy_rgb.shape}")
    print(f"IR input shape: {dummy_ir.shape}")
